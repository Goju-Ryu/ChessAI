...
namespace ChessAI.MoveSelection.MoveGeneration
{
    ...
    public class MoveCalculator : IMoveCalculator
    {
        ...
        {
            bool king = (piece.PieceType == Piece.King); // Also used as single distance boolean
            bool queen = (piece.PieceType == Piece.Queen);

            List<Move> moves = new List<Move>();
            if (piece.PieceType == Piece.Empty)
            {
                return moves;
            }
...
//              Console.WriteLine("PAWN   ");
        ...
        private List<Move> genPawnMoves(GameState state, Piece piece)
        {
            ...
            int[] posDelta = isLowIndexed
                ? new int[] { 0x10, 0x11, (0x10 - 0x01) }
                : new int[] { -0x10, -0x11, -(0x10 - 0x01) };
            int[] moveArr = StartPosition
                ? new int[]
                {
                    // if start position then the length of possible places should be 4
                ...
                : new int[]
                {
                    // if not the first move, then possible moves are just 3. 
                    piece.Position + posDelta[0],
                    piece.Position + posDelta[1],
                    piece.Position + posDelta[2]
                };
        ...
        List<Move> CastleSide(GameState state, Piece piece)
        {
            ...
            int left = isPos
                ? 0x00
                : Board.PositionConverter(
                    0x00); // the moveis only possible if the tower and king has not been moved, so assuming starting posionts    
